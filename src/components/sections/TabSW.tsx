...
import { Code, Building2, Calendar } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-3xl font-bold text-primary-cyan">{t('tabs.sw.title')}</h2>
      </div>

      {/* Core Skills */}
      <Card>
        <h3 className="text-2xl font-semibold mb-4 text-primary-cyan">{language === 'he' ? 'מיומנויות ליבה' : 'Core Skills'}</h3>
        <SkillsRadar skills={swData.skills[currentLang]} />
      </Card>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {swData.expertise[currentLang].map((area, index) => (
      ...
      <div className="space-y-4">
        ...
        {swData.workExperience[currentLang].map((job, index) => (
          <Card key={index}>
